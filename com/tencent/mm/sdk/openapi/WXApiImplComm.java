package com.tencent.mm.sdk.openapi;

import android.content.Context;
import android.content.Intent;
import android.content.pm.PackageInfo;
import android.content.pm.PackageManager;
import android.content.pm.PackageManager.NameNotFoundException;
import android.content.pm.Signature;
import com.tencent.mm.sdk.b.a;

class WXApiImplComm
{
  private static final String TAG = "MicroMsg.SDK.WXMsgImplComm";
  private static final String WX_APP_SIGNATURE = "";

  private WXApiImplComm()
  {
    throw new RuntimeException(getClass().getSimpleName() + " should not be instantiated");
  }

  public static boolean isIntentFromWx(Intent paramIntent, String paramString)
  {
    if (paramIntent == null)
      return false;
    return ((paramIntent = paramIntent.getStringExtra("wx_token_key")) != null) && (paramIntent.equals(paramString));
  }

  public static boolean validateAppSignatureForPackage(Context paramContext, String paramString, boolean paramBoolean)
  {
    if (!paramBoolean)
    {
      a.c("MicroMsg.SDK.WXMsgImplComm", "ignore wechat app signature validation");
      return true;
    }
    try
    {
      paramString = paramContext.getPackageManager().getPackageInfo(paramString, 64);
    }
    catch (PackageManager.NameNotFoundException localNameNotFoundException)
    {
      return false;
    }
    return validateAppSignature(paramContext, paramString.signatures, paramBoolean);
  }

  public static boolean validateAppSignature(Context paramContext, Signature[] paramArrayOfSignature, boolean paramBoolean)
  {
    if (!paramBoolean)
    {
      a.c("MicroMsg.SDK.WXMsgImplComm", "ignore wechat app signature validation");
      return true;
    }
    paramArrayOfSignature = (paramContext = paramArrayOfSignature).length;
    for (paramBoolean = false; paramBoolean < paramArrayOfSignature; paramBoolean++)
    {
      String str = paramContext[paramBoolean].toCharsString().toLowerCase();
      a.c("MicroMsg.SDK.WXMsgImplComm", "check signature:" + str);
      if (str.equals(""))
      {
        a.c("MicroMsg.SDK.WXMsgImplComm", "pass");
        return true;
      }
    }
    return false;
  }
}

/* Location:           /Users/wayliu/git/wechat-sdk-sample/amm_sdk_sample/libs/libammsdk 2.jar
 * Qualified Name:     com.tencent.mm.sdk.openapi.WXApiImplComm
 * JD-Core Version:    0.6.2
 */